...
export const fetchCookingOrders = createAsyncThunk(
  'order/waiter/fetchCookingOrders',
  (params = {}) => {
    return orderService
...
const waiterOrderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(handleSearch.fulfilled, (state, action) => {
      ...
      const groupByStatus = payload.data.data.reduce((group, order) => {
        const { status } = order;
    ...
    builder.addCase(fetchCookingOrders.fulfilled, (state, action) => {
      ...
      state.items = {
        ...state.items,
        cooking: [...payload.data.orders, ...state.items.cooking],
      };
      state.cooking.meta = payload.data.meta;
      state.cooking.params.page = payload.data.meta.current_page;
      state.cooking.params.perPage = payload.data.meta.per_page;
      state.cooking.error = '';
    });
