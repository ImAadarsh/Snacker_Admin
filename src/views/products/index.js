...
const ProductCategories = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    },
    {
      ...
      is_show: true,
      render: (_, row) => {
        return <div>{row.translation.title}</div>;
      },
    },
    {
  ...
  const paramsData = {
    ...
    shop_id: data?.branch_id?.value ? data?.branch_id?.value : DEMO_ADMIN,
    sort: data?.sort,
    status:
  ...
  const excelExport = () => {
    setDownloading(true);
    axios
      .get(`${BASE_URL}/api/v1/dashboard/admin/products/export`, {
        headers: {
          Authorization: `Bearer ${access_token}`,
        },
      })
      .then((res) => {
        const body = export_url + res.data.data.file_name;
  ...
  async function fetchBranches(search) {
    ...
    return restaurantService.getAll(params).then(({ data }) =>
      data.map((item) => ({
        label: item.translation?.title,
        value: item.id,
      }))
      ...
      <Card className='p-0'>
        <Space wrap size={[14, 20]}>
          ...
          <DebounceSelect
            ...
            style={{ minWidth: 180 }}
            onChange={(e) => handleFilter({ branch_id: e })}
            value={activeMenu.data?.branch_id}
          />
          <DebounceSelect
